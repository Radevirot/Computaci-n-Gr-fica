...
int main() {
	...
	do {
		...
		if (rotate) cs.model_angle += static_cast<float>(0.5f*dt);
		...
		if (enable_toon) {
			shader_lines.use();
			setMatrixes(window, shader_lines);
		...
		shader.setUniform("cameraPosition", cs.view_pos);
		glCullFace(GL_BACK);
