...
namespace ImGui {
	bool Combo(const char *label, int *current_item, const std::vector<std::string> &items) {
		return ImGui::Combo(label, current_item, 
							[](void* data, int idx) { 
								return (*reinterpret_cast<const std::vector<std::string>*>(data))[idx].c_str(); 
							}, 
...
Window::Window (int w, int h, const std::string & title, int flags, GLFWwindow *share_context_with) {
	if (windows_count==0) {
		glfwSetErrorCallback([](int code, const char *message){ 
			...
			if (code==GLFW_PLATFORM_ERROR or code==GLFW_FEATURE_UNAVAILABLE  ) {
				cg_info("GLFW code "+scode.str()+": "+message); 
			} else
#endif
			{
...
#if (GLFW_VERSION_MAJOR>=3) && (GLFW_VERSION_MINOR>=4)
	glfwWindowHint(GLFW_SCALE_FRAMEBUFFER, GLFW_FALSE); // evitar problemas en pantalla hidpi
#endif
...
void Window::disableImGui ( ) {
	cg_assert(imgui_context,"No ImGui context initialized for this window");
